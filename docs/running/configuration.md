...
`MAILER_SENDER` is the address this installation sends as; leaving it empty uses
`no-reply@` at the hostname that customer reaches you on.
...
while this is what the company running Xivi charges. A deployment picks it
...
!!! note "Why it was built this way"
    ...
    mail is synchronous and what the sender fallback is honest about (§8.7), and
    what the secret check exists for (§4.2) — is in `docs/architecture.md` of the
