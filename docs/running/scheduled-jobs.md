...
### Collecting support tickets

A support ticket is a customer's write too, so it lands in their own database
the same way, and this brings it to the operator's queue:

```cron
*/5 * * * *  cd /srv/xivi && bin/console tenant:support:collect
```

Five minutes rather than ten, because a person is waiting for an answer. The
operator's reply travels the other way with no collector at all: the customer
reads it directly, so it is on their screen the moment it is written.

### Keeping the usage figures fresh

The tenant list shows what each customer uses: users, last sign-in, records.
**Nothing collects those figures for you.** Until the command has run, every
...
weekly both tell the truth about themselves. A customer whose database cannot
be reached is recorded as *could not be read*, and the run carries on with the
rest.
...
!!! note "Why it was built this way"
    ...
    the provisioning design is §8.14, purchase requests are §8.15, support
    tickets are §8.17, and why a public surface never provisions directly is
    §8.12.
