...
every customer, provision new ones and decide what the modules cost. They are
not a promoted user of any customer, and they have no access to any customer's
records.
...
line, which is a deliberate choice rather than an omission: the surface that
can see every customer is not one anybody should be able to enrol themselves
into.
...
for a scripted run. An address that already has an operator is refused, because
creating one is not a way to change somebody's password;
`control:operator:password` is.
...
marked as revoked, and can be restored. What an operator did remains
attributable to somebody who still exists.
...
**An operator is not a user of any customer.** Nothing about this account
grants access to a customer's records; those are separate accounts in a
separate database.
